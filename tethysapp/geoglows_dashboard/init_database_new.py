...
from tethysapp.geoglows_dashboard.model import HydroSOSCategory, add_new_river_bulk, add_new_river_hydrosos_bulk, add_new_country
...
class HydroSOSDataInitializer:
    ...
    def insert_hydrosos_data(self):
        ...
        month_timeseries = xr.open_zarr('s3://rfs-v2/retrospective/monthly-timeseries.zarr', storage_options={'anon': True})
        xr.open_zarr('s3://rfs-v2/retrospective/monthly-timeseries.zarr', storage_options={'anon': True})
        linkno_raw = pd.read_parquet(f's3://rfs-v2/routing-configs/{self.vpu}/routing_parameters.parquet', storage_options={'anon': True})["river_id"].to_numpy()
        ...
        for i in range(len(river_ids)):
            ...
            river_df = flowdata[['time', 'year', 'month', river_id]].rename(columns={river_id: 'flow', 'time': 'date'}).dropna()
            # Calculate Long-Term Average (LTA)
            lta = river_df[(river_df['year'] >= self.stdStart) & (river_df['year'] <= self.stdEnd)].groupby('month')['flow'].mean()
            ...
            if i != 0 and (i % 400 == 0 or i == len(river_ids) - 1):
