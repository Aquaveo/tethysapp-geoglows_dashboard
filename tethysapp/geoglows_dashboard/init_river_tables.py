...
class HydroSOSRiverDataInitializer:
    ...
    def insert_hydrosos_data(self):
        ...
        df_stat = df_avg[['rivid', 'month', 'monthly_average']].merge(df_std[['rivid', 'month', 'monthly_std_dev']], on=['rivid', 'month'], how='outer')
        
        def get_hydrosos_data_sample(start_year, end_year):
            df_filtered_data = all_data["ds_grouped_avg"].sel(
                variable="Qout",
                time=(all_data["ds_grouped_avg"]["time"].dt.year >= start_year) & (all_data["ds_grouped_avg"]["time"].dt.year < end_year)
            ...
            df_merge = df_filtered_data.merge(df_stat[['rivid', 'month', 'monthly_average', 'monthly_std_dev']], on=['rivid', 'month'], how='left')
            df_merge['z_score'] = (df_merge['ds_grouped_avg'] - df_merge['monthly_average']) / df_merge['monthly_std_dev']
